...
class CampsiteReservationsConcurrentApplicationTests {
    ...
    public void mainThread() {
        ...
        try {
            ReservationRequest reservation = ReservationRequest.builder().email("mail").fullname("name")
                    .arrivalDate(LocalDate.now().plusDays(15))
                    .departureDate(LocalDate.now().plusDays(17)).build();
    ...
    public void secondThread() {
        try {
            ReservationRequest reservation = ReservationRequest.builder().email("mail2").fullname("name2")
                    .arrivalDate(LocalDate.now().plusDays(15))
                    .departureDate(LocalDate.now().plusDays(17)).build();
            reservationService.create(reservation);
        } catch (CampsiteConcurrentReservation e) {
    ...
    public void thirdThread() {
        try {
            ReservationRequest reservation = ReservationRequest.builder().email("mail3").fullname("name3")
                    .arrivalDate(LocalDate.now().plusDays(15))
                    .departureDate(LocalDate.now().plusDays(17)).build();
            reservationService.create(reservation);
        } catch (CampsiteConcurrentReservation e) {
